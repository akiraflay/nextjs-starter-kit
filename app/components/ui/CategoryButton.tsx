...
const CategoryButton: React.FC<CategoryButtonProps> = ({ icon, title, description, color }) => {
  ...
  return (
    ...
    >
      <Button 
        className={cn(
          "h-16 w-full flex flex-col items-center justify-center p-2",
          color,
          "hover:opacity-90 transition-all duration-300 ease-in-out rounded-lg shadow-md overflow-hidden"
        )}
      >
        {icon}
